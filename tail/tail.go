...
func RunUp(ctx context.Context, localClient *tailscale.LocalClient, authKey string) (retErr error) {
	var egg bool

	upArgs := upArgsT{
		server:       ipn.DefaultControlURL,
		acceptRoutes: acceptRouteDefault(runtime.GOOS),
	}
	...
	defer func() {
		if retErr == nil {
			checkUpWarnings(ctx, localClient)
		}
	}()
	...
	startLoginInteractive := func() { loginOnce.Do(func() { _ = localClient.StartLoginInteractive(ctx) }) }
...
func checkUpWarnings(ctx context.Context, localClient *tailscale.LocalClient) {
...
func RunStatus(ctx context.Context, localClient *tailscale.LocalClient) *ipnstate.Status {
	st, err := localClient.Status(ctx)
	if err != nil {
		panic(err)
	}
...
func RunUpCustom(ctx context.Context, localClient *tailscale.LocalClient, authKey string) error {
	st, err := localClient.Status(ctx)
	if err != nil {
		return err
	}

	simpleUp := st.BackendState != ipn.NeedsLogin.String()
